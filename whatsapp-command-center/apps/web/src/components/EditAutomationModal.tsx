...
export function EditAutomationModal({ open, onOpenChange, onSuccess, automation }: EditAutomationModalProps) {
  ...
  useEffect(() => {
    if (automation && open) {
      setFormData({
        ...
        triggerType: automation.triggerType || 'MESSAGE_RECEIVED',
        triggerConfig: automation.triggerConfig || { keyword: '' },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await automationsService.update(automation.id, {
        name: formData.name,
          ...
          <div className="space-y-4 pt-4 border-t">
            ...
            <div className="space-y-2">
              ...
              <select
                id="actionType"
                className="w-full px-3 py-2 border rounded-md bg-white dark:bg-gray-800"
                value={formData.actions[0]?.type || 'SEND_MESSAGE'}
                onChange={(e) => setFormData({
                  ...formData,
