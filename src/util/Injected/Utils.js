...
exports.LoadUtils = () => {
  ...
  window.WWebJS.forwardMessage = async (chatId, msgId) => {
    ...
    return await window.Store.ForwardUtils.forwardMessages({
      chat: chat,
      msgs: [msg],
      multicast: true,
      includeCaption: true,
  ...
  window.WWebJS.sendMessage = async (chat, content, options = {}) => {
    ...
    if (options.quotedMessageId) {
      ...
      !quotedMessage &&
        (quotedMessage = (
          await window.Store.Msg.getMessagesById([options.quotedMessageId])
        )?.messages?.[0]);
      if (quotedMessage) {
    ...
    if (isStatus) {
      ...
      const mediaUpdate = (data) =>
        window.Store.MediaUpdate(data, mediaOptions);
      const msg = new window.Store.Msg.modelClass({
        ...
        cannotBeRanked:
          window.Store.StatusUtils.canCheckStatusRankingPosterGating(),
      });

      // for text only
      const statusOptions = {
        color:
          (backgroundColor && window.WWebJS.assertColor(backgroundColor)) ||
          0xff7acca5,
      ...
      await window.Store.StatusUtils[
        isMedia ? 'sendStatusMediaMsgAction' : 'sendStatusTextMsgAction'
  ...
  window.WWebJS.processMediaData = async (
    ...
    if (
      shouldUseMediaCache &&
      mediaData.mediaBlob instanceof window.Store.OpaqueData
    ) {
      const formData = mediaData.mediaBlob.formData();
      window.Store.BlobCache.InMemoryMediaBlobCache.put(
        mediaObject.filehash,
        formData,
      );
    }
    ...
    const dataToUpload = {
      ...
      mediaType,
      ...(sendToChannel
        ? { calculateToken: window.Store.SendChannelMessage.getRandomFilehash }
        : {}),
    };
  ...
  window.WWebJS.getChat = async (chatId, { getAsModel = true } = {}) => {
    ...
    if (isChannel) {
      ...
      chat =
        ...
        (await window.Store.FindOrCreateChat.findOrCreateLatestChat(chatWid))
          ?.chat;
    }
  ...
  window.WWebJS.getChannelMetadata = async (inviteCode) => {
    ...
    return {
      ...
      isVerified:
        response.newsletterVerificationMetadataMixin.verificationState ===
        'verified',
    };
  };
  ...
  window.WWebJS.getChatModel = async (chat, { isChannel = false } = {}) => {
    ...
    if (chat.groupMetadata) {
      ...
      const groupMetadata =
        window.Store.GroupMetadata || window.Store.WAWebGroupMetadataCollection;
      await groupMetadata.update(chatWid);
      chat.groupMetadata.participants._models
        .filter((x) => x.id?._serialized?.endsWith('@lid'))
        .forEach(
          (x) => x.contact?.phoneNumber && (x.id = x.contact.phoneNumber),
        );
