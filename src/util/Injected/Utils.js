...
exports.LoadUtils = () => {
  ...
  window.WWebJS.getChats = async (offset = 0, limit) => {
    const chats = window.Store.Chat.getModelsArray();
    if (offset >= chats.length) return [];
    // if the limit is not a number, will return all chats
    // if the limit is number, maybe it's a NaN or Infinity, if it is NaN or Infinity will return all chats
    if (typeof limit !== "number" || isNaN(limit) || !isFinite(limit)) return window.WWebJS.getAllChats();
  ...
  window.WWebJS.getAllChats = async () => {
    const chats = window.Store.Chat.getModelsArray();
    const chatPromises = chats.map(chat => window.WWebJS.getChatModel(chat));
    return await Promise.all(chatPromises);
  }

  window.WWebJS.getChatsLength = async () => {
    const chats = window.Store.Chat.getModelsArray();
    return chats.length;
  };
