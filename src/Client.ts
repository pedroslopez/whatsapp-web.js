...
import { LegacySessionAuth } from "./authStrategies/LegacySessionAuth";
...
import { WhatsWebURL, DefaultOptions, Events, WAState, MessageAck } from "./util/Constants";
...
declare global {
    interface Window {
        ...
        qrChanged: (qr: string) => Promise<void>;
        onChangeMessageTypeEvent: (msg: any) => Promise<void>;
        onMessageAckEvent: (msg: any, ack: number) => Promise<void>;
        onMessageMediaUploadedEvent: (msg: any) => Promise<void>;
        onRemoveMessageEvent: (msg: any) => Promise<void>;
        onAppStateChangedEvent: (state: string) => Promise<void>;
        onBatteryStateChangedEvent: (state: string) => Promise<void>;
        onIncomingCall: (call: any) => Promise<void>;
        onAddMessageEvent: (msg: any) => Promise<void>;
        onChangeMessageEvent: (msg: any) => Promise<void>;
    }
}
...
export type ClientEvents = {
    ...
    auth_failure: (payload: any) => void;
    authenticated: (payload: any) => void;
    disconnected: (reason: WAState | "NAVIGATION" | string) => void;
    group_join: (notification: GroupNotification) => void;
    group_leave: (notification: GroupNotification) => void;
    group_update: (notification: GroupNotification) => void;
    message_revoke_me: (message: Message) => void;
    message_revoke_everyone: (message: Message, revokedMsg?: Message) => void;
    message_ack: (message: Message, ack: MessageAck) => void;
    media_uploaded: (message: Message) => void;
    change_state: (state: WAState) => void;
    change_battery: (batteryInfo: {battery: number; plugged: boolean}) => void;
 ...
 * @param {string} options.restartOnAuthFail  - This option should be set directly on the LegacySessionAuth.
...
export class Client extends (EventEmitter as new () => TypedEmitter<ClientEvents>) {
    ...
    async initialize() {
        ...
        if (needAuthentication) {
            ...
            await page.evaluate(
                function (selectors) {
                    const qr_container: HTMLElement = document.querySelector(
                    ...
                    const obs = new MutationObserver((muts) => {
                        muts.forEach((mut) => {
                            ...
                            if (
                                ...
                                window.qrChanged(target.dataset.ref);
                            }
    ...
    async sendMessage(
        ...
        const newMessage = await this.pupPage.evaluate(
            async (chatId: string, message: string, options, sendSeen) => {
    ...
    async searchMessages(
        query,
        options: { page?: number; limit?: number; chatId?: string } = {}
    ) {
    ...
    async getBlockedContacts() {
        const blockedContacts = await this.pupPage.evaluate(() => {
            let chatIds = window.Store.Blocklist.models.map(
                (a) => a.id._serialized
            );
            return Promise.all(
