...
export class Message extends Base {
     ...
     */
    async forward(chat: string | Chat) {
            ...
            async (msgId: string) => {
                ...
                try {
                    const decryptedMedia =
                        await window.Store.DownloadManager.downloadAndDecrypt({
     ...
     */
    async delete(everyone?: boolean) {
        await this.client.pupPage.evaluate(
            (msgId, everyone) => {
                let msg = window.Store.Msg.get(msgId);

                if (everyone && msg.id.fromMe && msg._canRevoke()) {
                    return window.Store.Cmd.sendRevokeMsgs(msg.chat, [msg], {
                        type: "Sender",
                    });
                }
