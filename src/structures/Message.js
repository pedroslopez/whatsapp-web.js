  ...
  _patch(data) {
    ...
    this.inviteV4 =
      data.type === MessageTypes.GROUP_INVITE
        ? {
            ...
            fromId:
              typeof data.from === 'object' && '_serialized' in data.from
                ? data.from._serialized
                : data.from,
            toId:
              typeof data.to === 'object' && '_serialized' in data.to
                ? data.to._serialized
                : data.to,
          }
        : undefined;
    ...
    if (this.type === MessageTypes.POLL_CREATION) {
      ...
      this.messageSecret = data.messageSecret
        ? Object.keys(data.messageSecret).map((key) => data.messageSecret[key])
        : [];
    }
