...
class Client extends EventEmitter {
     ...
     */
    async getChats(offset = 0, limit = 20) {
        const chats = await this.pupPage.evaluate(async (offset = 0, limit = 20) => {
            return await window.WWebJS.getChats(offset, limit);
        }, offset, limit);
    ...
    async getAllChats() {
        const chats = await this.pupPage.evaluate(async () => {
            return await window.WWebJS.getAllChats();
        });
    ...
    /**
     * Get the length of all current chat instances
     * @returns {number}
     */
    async getChatsLength() {
        const length = await this.pupPage.evaluate(async () => {
            return await window.WWebJS.getChatsLength();
        });
