...
class Client extends EventEmitter {
  ...
  async attachEventListeners() {
    ...
    await this.pupPage.evaluate(() => {
      ...
      const callCollection =
        (window.Store && window.Store.Call) ||
        (window.Store && window.Store.WAWebCallCollection);
      if (callCollection && typeof callCollection.on === 'function') {
        callCollection.on('add', (call) => {
      ...
      pollVoteModule.bulkUpsert = (async (...args) => {
        const votes = await Promise.all(
          args[0].map(async (vote) => {
            ...
            if (!parentMessage) {
              const fetched = await window.Store.Msg.getMessagesById([
      ...
      async (chatId, channelId, options) => {
        ...
        return await window.Store.SendChannelMessage.sendNewsletterAdminInviteMessage(
          chat,
          {
            ...
            base64Thumb:
              await window.WWebJS.getProfilePicThumbToBase64(channelWid),
          },
        );
      },
      chatId,
      ...
      const pinnedMsgs = (
        await Promise.all(
          msgs
            ...
            .map(async (msg) => {
              const res = await window.Store.Msg.getMessagesById([
                msg.parentMsgKey,
              ]);
          ...
          createGroupResult = await window.Store.GroupUtils.createGroup(
            {
              ...
              restrict:
                options.isRestrict !== undefined ? !options.isRestrict : false,
        ...
        countryCodes =
          countryCodes.length === 1 && countryCodes[0] === currentRegion
            ...
            : countryCodes.filter((code) =>
                Object.keys(window.Store.ChannelUtils.countryCodesIso).includes(
                  code,
                ),
              );
  ...
  async revokeStatusMessage(messageId) {
    return await this.pupPage.evaluate(async (msgId) => {
      ...
      return await window.Store.StatusUtils.sendStatusRevokeMsgAction(
        status,
        msg,
      );
    }, messageId);
  ...
  async getPollVotes(messageId) {
    ...
    const pollVotes = await this.pupPage.evaluate(async (msg) => {
      ...
      let pollVotes = await window.Store.PollsVotesSchema.getTable().equals(
        ['parentMsgKey'],
        msgKey.toString(),
      );
