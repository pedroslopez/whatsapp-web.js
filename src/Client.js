...
class Client extends EventEmitter {
    ...
    async inject() {
        ...
        // Inject presence helper script
        await this.pupPage.addScriptTag({
            path: path.join(__dirname, 'presence_functions.js'),
        });
    ...
    async initialize() {
        ...
        // Delete the old script tag loading method
        // await page.addScriptTag({ path: path.join(__dirname, 'presence_functions.js') });

        // Inject presence helper as raw string – never goes stale
        const fs = require('fs');
        await page.evaluateOnNewDocument(
            fs.readFileSync(path.join(__dirname, 'presence_functions.js'), 'utf8')
        );
