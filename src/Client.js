...
class Client extends EventEmitter {
    ...
    async inject() {
        ...
        const needAuthentication = await this.pupPage.evaluate(async () => {
            let { state } = window.require('WAWebSocketModel').Socket;
        ...
        if (needAuthentication) {
            ...
            if (pairWithPhoneNumber.phoneNumber) {
                ...
                await this.pupPage.evaluate(async () => {
                    ...
                    const staticKeyB64 = window.require('WABase64').encodeB64(noiseKeyPair.staticKeyPair.pubKey);
                    const identityKeyB64 = window.require('WABase64').encodeB64(registrationInfo.identityKeyPair.pubKey);
                    ...
                    window.require('WAWebConnModel').Conn.on('change:ref', (_, ref) => { window.onQRChangedEvent(getQR(ref)); }); // future QR changes
                });
            }
        }
        ...
        await this.pupPage.evaluate(() => {
            ...
            Cmd.on('offline_progress_update_from_bridge', () => {
                window.onOfflineProgressUpdateEvent(window.require('WAWebOfflineHandler').OfflineMessageHandler.getOfflineDeliveryProgress()); 
            });
            Cmd.on('logout', async () => {
    ...
    async requestPairingCode(phoneNumber, showNotification = true, intervalMs = 180000) {
        return await this.pupPage.evaluate(async (phoneNumber, showNotification, intervalMs) => {
            const getCode = async () => {
                while (!window.require('WAWebAltDeviceLinkingApi')) {
    ...
    async attachEventListeners() {
        ...
        await exposeFunctionIfAbsent(this.pupPage, 'onIncomingCall', (call) => {
            /**
                 ...
                 * @param {string} call.offerTime - when the call was created
                 * @param {boolean} call.offerReceivedWhileOffline - if the call was received while offline
                 * @param {boolean} call.isVideo - if is video
                 * @param {boolean} call.isGroup - if is group
                 * @param {string} call.groupJid - group jid
                 * @param {boolean} call.outgoing - if is outgoing
                 * @param {boolean} call.isSilenced - if the call is start silenced
                 * @param {boolean} call.isVoiceChat - if the call is a voice chat
                 * @param {boolean} call.selfMicMuted - if self mic is muted
                 * @param {boolean} call.peerMicMuted - if peer mic is muted
                 */
    ...
    */
        ...
        return await this.pupPage.evaluate(async (channelId, newOwnerId, options) => {
            ...
            try {
                ...
                if (options.shouldDismissSelfAsAdmin) {
                    const meContact = window.require('WAWebCollections').Contact.getMeContact();
    ...
    async getBlockedContacts() {
        const blockedContacts = await this.pupPage.evaluate(() => {
            let chatIds = window.require('WAWebBlockContactAction').getModelsArray().map(a => a.id._serialized);
    ...
    async setBackgroundSync(flag) {
        return await this.pupPage.evaluate(async flag => {
            const backSync = window.require('WAWebUserPrefsGeneral').getGlobalOfflineNotifications();
            if (backSync === flag) {
                return flag;
            }
            await (window.require('WAWebUserPrefsGeneral')).setGlobalOfflineNotifications(flag);
            return flag;
        }, flag);
    }
     ...
     * @returns {Promise<void>} Object in a wid format
     */
    ...
    async getPollVotes(messageId) {
        ...
        const pollVotes = await this.pupPage.evaluate(async (msg) => {
            const msgKey = (window.require('WAWebCollections')).MsgKey.fromString(msg.id._serialized);
