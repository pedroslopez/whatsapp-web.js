 ...
 */
export class LegacySessionAuth extends BaseAuthStrategy {
    ...
    constructor({
        session,
        restartOnAuthFail,
    }: { session?: ClientSession; restartOnAuthFail?: boolean } = {}) {
        super();
    ...
    async afterBrowserInitialized() {
        if (this.session) {
            await this.client.pupPage.evaluateOnNewDocument((session: ClientSession) => {
